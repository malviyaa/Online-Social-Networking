...
consumer_key = 'T0c87spDABai28fS0aLvMBGOX'
consumer_secret = 'hyAE5bGPdT3KziGtcDTv8SzwZTpE13ZqJzPtNtntfyUyb2IOJG'
access_token = '2654489490-UibYLeIyk9HDfcjeUkLIjP3npXMtTsaLk2rSYPU'
access_token_secret = 'S41olrzpHJNnfOgFAjdNWUep55j6Efsq8pVTC2i6yp1AR'
...
def get_users(twitter, screen_names):
    ...
    request= robust_request(twitter,resource,params,5)
...
def get_friends(twitter, screen_name):
    ...
    idslimt= sorted(ids[:5000], key=int)
    return idslimt
...
def add_all_friends(twitter, users):
    ...
    for dictionary in users:
        ids= get_friends(twitter, dictionary['screen_name'])
        dictionary['friends']=ids
    users= dictionary
...
def print_num_friends(users):
    ...
    for x in range(0, len(users)):
        candiateList.append(users[x].get('screen_name'))
    candiateList= sorted(candiateList)
    
    for i in candiateList: 
        print (i,len(users[j].get('friends')))
        j=j+1
...
def count_friends(users):
    ...
    for i in range(0, len(users)):
        dict={'friends':users[i].get('friends')}
        counts.update(dict['friends'])
...
def friend_overlap(users):
    ...
    for x in users:
        candidateList.append(x['screen_name'])
    ...
    for i in range(0,len(candidateList)):
        ...
        for j in range(i+1,len(candidateList)):
            ...
            dict={}
            for x in users:
                if x['screen_name']==candidateList[i]:
                    dict={'friends':x['friends']}
                    counts.update(dict['friends'])
                if x['screen_name']==candidateList[j]:
                    dict={'friends':x['friends']}
                    counts.update(dict['friends'])
            commonFollower= counts.most_common()
            
            for tup in commonFollower:
                if tup[1]>1:
                    total=total+1     
            friendList.append((candidateList[i],candidateList[j],total))
   
    return sorted(friendList, key=lambda tup: tup[2],reverse=True)   
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for k in users:
        if k.get('screen_name')=='HillaryClinton':
            dict={'friends':k.get('friends')}
            counts.update(dict['friends'])
        if k.get('screen_name')=='realDonaldTrump':
            dict={'friends':k.get('friends')}
            counts.update(dict['friends'])         
    commonFollower= counts.most_common(1)

    resource='users/show'
    for x in commonFollower:
        params={'id':x[0]}
        
    request= robust_request(twitter,resource,params,5)
    user = [r for r in request]   
    return user[0].get('screen_name')
...
def draw_network(graph, users, filename):
    ...
    fig = plt.figure()
    nx.draw_networkx(graph,labels=labels,width=0.1,font_color='k',edge_color='b',node_size=20)

...
def partition_girvan_newman(graph, max_depth):
    ...
    dictionary = sorted(betweenness.items(), key = lambda x: (-x[1],x[0][0],x[0][1]))
    
    for edge_to_remove in dictionary:
        if(len(components) > 1):
            break
...
def make_training_graph(graph, test_node, n):
    ...
    """
    G = graph.copy()
    neighbors = sorted(G.neighbors(test_node))
    for i in range(0,n):
        edge_to_remove=(neighbors[i],test_node)
...
def jaccard(graph, node, k):
    ...
    common_jaccard= sorted(scores, key=lambda x: (-x[1],x[0]))[:k]
    return common_jaccard
    ###TODO
    pass
...
def evaluate(predicted_edges, graph):
    ...
    edge = 0
    for count_edge in predicted_edges:
        if(graph.has_edge(count_edge[0],count_edge[1])):
            edge+=1

    return edge/len(predicted_edges)
###TODO
    pass
